...
function parseCurrency (_sAmount, _currencyLocale){ 	    if (!_sAmount) {	    	return NaN;	    }	    _sAmount = 	    	(function removeCurrencySymbols(curr){	    		var countryCodes = [ 'US', 'GB', 'CA' ];	            var currencySymbols = [ '$', '\u00A3' ];	            for (var i = 0; i < countryCodes.length; i++) {	            	var countryCode = countryCodes[i];	            	for (var j = 0; j < currencySymbols.length; j++) {	            		var currencySymbol = currencySymbols[j];	            		curr = curr.replace(countryCode + currencySymbol, '').replace(currencySymbol + countryCode, '');	            	}	            }	            for (var i = 0; i < currencySymbols.length; i++) {	        		var currencySymbol = currencySymbols[i];	        		curr = curr.replace(currencySymbol, '');	        	}	            return curr;	    	})(_sAmount);	    _sAmount = jQuery.trim(_sAmount);	    var negativeSignDetectedAndRemoved = (_sAmount.match(/^-.*$/g) != null);	    if (negativeSignDetectedAndRemoved) {	    	_sAmount = _sAmount.replace(/-(.*)/g,"$1");	    }	    var accountantNegativeSignDetectedAndRemoved = (_sAmount.match(/^\(.*\)$/g) != null);	    if (accountantNegativeSignDetectedAndRemoved) {	    	_sAmount = _sAmount.replace(/\((.*)\)/g,"$1");	    }	    	    var currencyLocale = _currencyLocale;	    if (currencyLocale === 'fr_CA') {	    	_sAmount = _sAmount.replace(/,/g,".");	    	_sAmount = _sAmount.replace(/\s/g,",");	    }	   	    _sAmount = _sAmount.replace(/,/g,"");	    	    var retVal = _sAmount;	    retVal = Math.round(retVal * 100);	    retVal = retVal * ((negativeSignDetectedAndRemoved) ? -1 : 1) * ((accountantNegativeSignDetectedAndRemoved) ? -1 : 1);	    	    return retVal;}
...
function showTimedOutDialog(expireText){keepAliveDialog.cfg.queueProperty('icon',YAHOO.widget.SimpleDialog.ICON_WARN);keepAliveDialog.setBody(expireText);keepAliveDialog.cfg.queueProperty('close',true);var logIn=function(){window.location.href=window.location.href;this.hide();}
keepAliveDialog.cfg.queueProperty('close','true');var buttons=[{text:'Log Back In',handler:logIn}];keepAliveDialog.cfg.queueProperty('buttons',buttons);keepAliveDialog.render(document.body);keepAliveDialog.show();}
...
var expireText='Your Convio session will expire in '+remainingText+' unless you continue working.';showTimingOutDialog(expireText,callback.argument.confirmURL,callback.argument.logoutURL);}
else{clearInterval(keepAliveTimer);var expireText='Your Convio session expired at '+formatTime(new Date(expireTime))+'.';showTimedOutDialog(expireText);}
keepAliveDialog.cfg.queueProperty('zIndex',400000);keepAliveDialog.cfg.refireEvent('zIndex');}
function keepAlivePoll(_servletPath,_confirmURL,_logoutURL,_sessionId){var callback={success:showKeepAliveDialog,argument:{confirmURL:_confirmURL,logoutURL:_logoutURL}};YAHOO.util.Connect.asyncRequest('POST',_servletPath,callback,'action=time&id='+_sessionId);}
function keepAlive2(_confirmURL,_logoutURL,_sessionId,_sessionTag,_adminPath){if(WCAGState==1){playMIDI();}
var servletPath=_adminPath+'SessionTimeout';Y.use('yui2-cookie','yui2-json','yui2-connection','yui2-container','yui2-button',function(){YAHOO.util.Cookie.set(_sessionTag,'override_session_'+_sessionId,{path:servletPath});initKeepAliveDialog();var pollExpression='keepAlivePoll("'+servletPath+'", "'+_confirmURL+'", "'+_logoutURL+'", "'+_sessionId+'")';eval(pollExpression);keepAliveTimer=setInterval(pollExpression,10000);});}
function forceKeepAlive2(_confirmURL){Y.use('yui2-connection',function(){var callback={};YAHOO.util.Connect.asyncRequest('GET',_confirmURL,callback);});}
...
str=str.substring(1,str.length);if(!frontOnly){while(str.substr(str.length-1,1)==' ')
...
function enhanceDomToPostLatin1EncodedData() {
	// no-op ...see http://bugzilla.corp.convio.com/show_bug.cgi?id=59592#c44
}